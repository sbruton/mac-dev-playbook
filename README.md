...
After applications are installed, the playbook configures a few macOS defaults: [`duti`] assigns [iTerm2] as the handler for `.command` scripts, the built-in AirPlay receiver is disabled for privacy, the system clock switches to 24-hour time, and the time zone auto-updates based on location.
